    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for name, hyps in hyps_dict.items():
            ...
            for cut_id, hyp_words, ref_text in zip(cut_ids, hyps, texts):
                ...
                ref_words = bpe_model.id_to_piece(bpe_model.tokenize(ref_text))[1:]
                if len(hyp_words)!=0:
                    hyp_words = bpe_model.id_to_piece(bpe_model.tokenize(hyp_words[0]))[1:]
...
def main():
    ...
    max_token_id = 126 #max(lexicon.tokens)
    num_classes = None #max_token_id + 1  # +1 for the blank

    params.vocab_size = 127#num_classes
    ...
    test_doreco_cuts = librispeech.test_doreco_cuts()
    test_ucla_cuts = librispeech.test_ucla_cuts()
    test_aishell_cuts = librispeech.test_aishell_cuts()
    test_mls_german_cuts = librispeech.test_mls_german_cuts()
    test_mls_italian_cuts = librispeech.test_mls_italian_cuts()
    test_mls_french_cuts = librispeech.test_mls_french_cuts()
    test_mls_spanish_cuts = librispeech.test_mls_spanish_cuts()
    test_mls_portuguese_cuts = librispeech.test_mls_portuguese_cuts()
    test_mls_dutch_cuts = librispeech.test_mls_dutch_cuts()
    test_buckeye_cuts = librispeech.test_buckeye_cuts()
    test_l2_arctic_cuts = librispeech.test_l2_arctic_cuts()
    test_l2_arctic_preceived_cuts = librispeech.test_l2_arctic_preceived_cuts()
    test_azerbaijani_cuts = librispeech.test_azerbaijani_cuts()

    test_clean_dl = librispeech.test_dataloaders(test_clean_cuts)
    test_other_dl = librispeech.test_dataloaders(test_other_cuts)
    test_doreco_dl = librispeech.test_dataloaders(test_doreco_cuts)
    test_ucla_dl = librispeech.test_dataloaders(test_ucla_cuts)
    test_aishell_dl = librispeech.test_dataloaders(test_aishell_cuts)
    test_mls_french_dl = librispeech.test_dataloaders(test_mls_french_cuts)
    test_mls_german_dl = librispeech.test_dataloaders(test_mls_german_cuts)
    test_mls_italian_dl = librispeech.test_dataloaders(test_mls_italian_cuts)
    test_mls_portuguese_dl = librispeech.test_dataloaders(test_mls_portuguese_cuts)
    test_mls_spanish_dl = librispeech.test_dataloaders(test_mls_spanish_cuts)
    test_mls_dutch_dl = librispeech.test_dataloaders(test_mls_dutch_cuts)
    test_buckeye_dl = librispeech.test_dataloaders(test_buckeye_cuts)
    test_l2_arctic_dl = librispeech.test_dataloaders(test_l2_arctic_cuts)
    test_l2_arctic_preceived_dl = librispeech.test_dataloaders(test_l2_arctic_preceived_cuts)
    test_azerbaijani_dl = librispeech.test_dataloaders(test_azerbaijani_cuts)

    test_sets = ["test-clean", "test-other","doreco", "ucla", "aishell", "mls-french", "mls-german", "mls-italian", "mls-portuguese", "mls-spanish", "mls-dutch", "buckeye", "l2-arctic", "l2-arctic-perceived","azerbaijani"]
    test_cuts = [test_clean_dl, test_other_dl, test_doreco_dl, test_ucla_dl, test_aishell_dl, test_mls_french_dl, test_mls_german_dl, test_mls_italian_dl, test_mls_portuguese_dl, test_mls_spanish_dl, test_mls_dutch_dl,test_buckeye_dl,test_l2_arctic_dl,test_l2_arctic_preceived_dl,test_azerbaijani_dl]


    for test_set, test_dl in zip(test_sets, test_cuts):
