...
def main():
    ...
    params.res_dir = params.exp_dir / "unsup_data_decoded"
    ...
    shars = [i for i in os.listdir('/scratch/lingjzhu_root/lingjzhu1/lingjzhu/unsup_train') if 'jsonl' in i]
    

    for shar in shars:
        cut = os.path.join('/scratch/lingjzhu_root/lingjzhu1/lingjzhu/unsup_train',shar)
        recording = cut.replace('cuts','recording').replace('jsonl.gz','tar')
        test_set = shar.replace('cuts.','').replace('.jsonl.gz','')
        
        test_cuts = CutSet.from_shar(
            fields={
                "cuts": [cut],
                "recording": [recording]
            })
