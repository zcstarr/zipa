...
model = WhisperForConditionalGeneration.from_pretrained("/scratch/lingjzhu_root/lingjzhu1/lingjzhu/checkpoint-3184",torch_dtype=torch.bfloat16,attn_implementation="flash_attention_2")
model.eval().to("cuda")
...
test_sets = ['ucla']
test_cuts = [test_ucla_cuts]

output_path = os.path.join('/scratch/lingjzhu_root/lingjzhu1/lingjzhu/asr_exp','whisper-ppt')
...
for name, cutset in zip(test_sets, test_cuts):
    ...
    with open(os.path.join(output_path,f"{name}.jsonl"),'w') as out:
        pfers = []
        dl = test_dataloaders(cutset)
        for batch in tqdm(dl):
            features = [i.tolist() for i in batch["inputs"]]
            texts = batch["supervisions"]["text"]
            input_values = processor(features, sampling_rate=16000,return_tensors="pt").input_features
            
            # retrieve logits
            generated_ids = model.generate(inputs=input_values.bfloat16().to('cuda'),num_beams=1,max_new_tokens=300)

            transcriptions = processor.batch_decode(generated_ids, skip_special_tokens=True)

            for tr, pr in zip(texts,transcriptions):
            # take argmax and decode
                pr = "".join(pr).replace("͡",'').replace("g","ɡ")
                tr = "".join(tr).replace("g","ɡ")
                pfer = dst.feature_edit_distance(tr,pr)
                pfers.append(pfer)
                out.write(json.dumps({"ground truth": tr, "predicted": pr, "pfer":pfer},ensure_ascii=False)+"\n")
